...
The local continued-fraction block API is factored through the `Blocks`
modules:

* `Blocks.Basic` contains principal/intermediate block objects;
* `Blocks.ContinuantBounds` contains generic continuant growth and endpoint
  lemmas;
* `Blocks.Selected` contains parity-selected block combinatorics;
* `Blocks.Visible` contains finite visible truncations and exponent
  functionals.

This file now carries the asymptotic comparison proofs and the Euler-facing
consequences built on that API.

The main asymptotic bridges are formalized below, with a few reusable
predicate-style APIs used to state the writeup's formulas cleanly:

* `HasCanonicalBlockGrowthFormula`;
* `HasIrrationalityMeasureFromCF`;
* `HasEulerPartialQuotients`.

Together with the finite additive-combinatorial bridge, these APIs package the
canonical block exponent, irrationality-measure, visible-set, and Euler
consequences while keeping the compiled project proof-hole-free.
-/
...
/-!
The standard continued-fraction irrationality-measure bridge is packaged as:
...
After introducing `denominatorRatioExponent`, the proof is organized through
two precise statements proved later in this file:
